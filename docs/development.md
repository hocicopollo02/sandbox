...
```bash
go build -ldflags "-X github.com/hocicopollo02/sandbox/cmd.Version=1.1.0 -X github.com/hocicopollo02/sandbox/cmd.Commit=$(git rev-parse --short HEAD) -X github.com/hocicopollo02/sandbox/cmd.BuildDate=$(date -u +%Y-%m-%dT%H:%M:%SZ)" -o sandbox .
```
...
4. Crea y publica un tag SemVer, por ejemplo `v1.1.0`.
5. Comprueba `go install github.com/hocicopollo02/sandbox@v1.1.0`.
